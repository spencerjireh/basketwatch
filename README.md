...
- **Twelve stores run as Studio collectors** (the `c_*` IDs above). Four
  ...
  plain HTTP; requests route through Web Unlocker for the sites that block
  scraping.
...
- **The heal loop repairs collectors through Studio.** When a store changes
  ...
  rewrites the collector's extraction recipe in place. The fleet is
  reproducible from
  [`docs/collector-manifest.json`](docs/collector-manifest.json), which
  registers every store with the exact seed URL and instruction used to
  create its collector.
